...
describe Slather::CoverageFile do
  ...
  describe "#initialize" do
    it "should convert the provided path string to a Pathname object, and set it as the gcno_file_pathname" do
      expect(line_coverage_file.gcno_file_pathname.exist?).to be_truthy
      expect(line_coverage_file.gcno_file_pathname.basename.to_s).to eq("fixtures.gcno")
    end
  end

  describe "#source_file_pathname" do
    it "should return the path to the coverage files's source implementation file" do
      expect(line_coverage_file.source_file_pathname).to eq(fixtures_project["fixtures/fixtures.m"].real_path)
    end

    it "should look in the source_directory if it has been set on the project" do
      line_coverage_file = Slather::CoverageFile.new(fixtures_project, "fixtures.m")
    ...
    it "should look in the source_directory if it has been set on the project" do
      ...
      expect(line_coverage_file.source_file_pathname).to be_nil
    end
  ...
  describe "#source_file" do
    it "should return a file object for the source_file_pathname" do
      file = line_coverage_file.source_file
      expect(file.kind_of?(File)).to be_truthy
      expect(Pathname(file.path)).to eq(line_coverage_file.source_file_pathname)
    end
  end
...
@end
OBJC
      expect(line_coverage_file.source_data).to eq(expected)
    end
  end
  ...
  describe "source_file_pathname_relative_to_repo_root" do
    it "should return a pathname to the source_file, relative to the root of the repo" do
      expect(line_coverage_file.source_file_pathname_relative_to_repo_root).to eq(Pathname("spec/fixtures/fixtures/fixtures.m"))
    end
  end
  ...
  describe "#ignored" do
    it "should return true if the source_file_pathname globs against anything in the project.ignore_list" do
      line_coverage_file.project.ignore_list = ["*spec*", "*test*"]
      expect(line_coverage_file.ignored?).to be_truthy
    end

    it "should return false if the source_file_pathname does not glob against anything in the project.ignore_list" do
      line_coverage_file.project.ignore_list = ["*test*", "*XCTest*"]
      expect(line_coverage_file.ignored?).to be_falsy
    end
  end

  describe "gcov_data" do
    it "should process the gcno file with gcov and return the contents of the file" do
      expect(line_coverage_file.gcov_data.include?("1:   15:    NSLog(@\"tested\");")).to be_truthy
    end
  end
  ...
  describe "source_file_basename" do
    it "returns the base name of the source file" do
      expect(branch_coverage_file.source_file_basename).to eq("Branches")
    end
  end
